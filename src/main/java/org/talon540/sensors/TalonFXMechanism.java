...
public class TalonFXMechanism {
   ...
   * @param gearRatio gear ratio between the motor and the object. If it is not linear i.e.
   *     drivetrain gearbox is 54:20, enter 54.0/20.0
   */
  public TalonFXMechanism(
      @NotNull TalonFXSensorCollection collection, double radius, double gearRatio) {
   ...
   *
   * @param collection {@link TalonFXSensorCollection} from the motor controller
   * @param radius radius of the attached item in meters Note: Assumes there is no gearbox attached
   *     (1:1) gear ratio
   */
  ...
  public double getAngularVelocity() {
    return UnitUtil.Falcon500VelocityToAngularVelocity(
        collection.getIntegratedSensorVelocity(), kGearRatio);
  }

  /** Get the linear velocity in meters per second */
  public double getLinearVelocity() {
    return UnitUtil.Falcon500VelocityToLinearVelocity(
        collection.getIntegratedSensorVelocity(), kRadius, kGearRatio);
  }
  ...
  public double getPosition() {
    return (collection.getIntegratedSensorPosition() / kGearRatio)
        * ((2 * Math.PI * kRadius) / 2048.0);
  }
