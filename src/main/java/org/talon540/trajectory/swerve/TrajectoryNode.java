...
public class TrajectoryNode {
    ...
    /**
     * 
     * @param time timepoint of this point in the trajectory in {@code seconds}
     ...
     * @param data
     * @implNote must be of len 7, even if a datapoint is 0 (which it shouldn't be) add a zero for blank spaces or an exception is thrown
     */
    ...
    public TrajectoryNode interpolateNode(TrajectoryNode ceilingNode, double pos) {
        ...
        Pose2d interpolatedPose2d = (((endVector.substractVector(startVector)).multiplyVectorByScale(pos)).addVector(startVector)).toPose2d();
        // Pose2d interpolatedPose2d = position.interpolate(ceilingNode.position, pos); // idk if this works
        Vector3d interpolatedVector3d = ((ceilingNode.velocity.substractVector(velocity)).multiplyVectorByScale(pos)).addVector(velocity);
